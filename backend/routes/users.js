...
router.put("/:id", authenticate, authorize("admin"), (req, res) => {
  ...
  const demotingFromAdmin = target.role === "admin" && role && role !== "admin";
  const suspendingAdmin = target.role === "admin" && status && status !== "active";

  if (isSelf && status && status !== "active") {
    return res.status(400).json({ message: "You cannot suspend your own account." });
  }
  if (isSelf && demotingFromAdmin) {
    return res.status(400).json({ message: "You cannot change your own admin role." });
  }
  if ((demotingFromAdmin || suspendingAdmin) && !isSelf) {
    const otherActiveAdmins = users.filter((u) => u.role === "admin" && u.status === "active" && u.id !== target.id);
